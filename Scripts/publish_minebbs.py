...
def publish_minebbs(resource_id: int, token: str, file_path: str, version: str, change_file: str) -> bool:
    manager = UpdateManager(resource_id, token, version)
    
    file_key = manager.upload(file_path)
    if file_key:
        manager.updateFile(change_file, file_key)
    else:
...
if __name__ == '__main__':
    if False:
        ...
        args = argparse.Namespace(
            file="x64\LLCheckBag-v1.2.4.zip",
            change="CHANGELOG.md",
            version="1.2.4",
            token="",
        )
