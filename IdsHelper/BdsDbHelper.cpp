...
Tag* readFromBinary2(string& bin, size_t& offset) {
    // bits version
...
Tag* readFromBinary(string& bin, size_t& offset, bool isLittleEndian) {
    void* vtbl;
    if(isLittleEndian)
...
Tag* readFromBinary(string& bin, bool isLittleEndian) {
    size_t offset = 0;
    return readFromBinary(bin, offset, isLittleEndian);
}

vector<Tag*> readAllFromBinary(string& bin, bool isLittleEndian) {
    vector<Tag*> tags;
    size_t offset = 0;
    while (offset < bin.length()) {
        auto tag = readFromBinary(bin, offset, isLittleEndian);
        if (offset < 4)
            return tags;
...
string writeAllToBinary(vector<Tag*>  tags, bool isLittleEndian) {
    string bin;
    for (auto tag : tags) {
        bin += writeToBinary(tag, isLittleEndian);
    }
    return bin; 
}
